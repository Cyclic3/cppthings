...
#define CPPTHINGS_IMPL_SHALLOW_EQ_NOEXCEPT(TYPE, ...) \
  inline bool operator==(const TYPE& other) const noexcept { return ::cppthings::check_shallow_equal(*this, other __VA_OPT__(,) CPPTHINGS_FOR_EACH_ARG(_CPPTHINGS_MEMBER_PTR, TYPE, __VA_ARGS__));  } \
  inline bool operator!=(const TYPE& other) const noexcept { return !::cppthings::check_shallow_equal(*this, other __VA_OPT__(,) CPPTHINGS_FOR_EACH_ARG(_CPPTHINGS_MEMBER_PTR, TYPE, __VA_ARGS__));  }
#else
#define CPPTHINGS_IMPL_SHALLOW_EQ(TYPE, ...) \
...
#define CPPTHINGS_EZ_CONSTRUCTOR(NAME, ...) \
  NAME(CPPTHINGS_FOR_EACH(_CPPTHINGS_MAKE_PARAMS, __VA_ARGS__)) : CPPTHINGS_FOR_EACH(_CPPTHINGS_MAKE_CONSTRUCTOR, __VA_ARGS__) {}
